...
std::shared_ptr<tr_peerIo> tr_peerIo::create(
    tr_session* session,
    std::unique_ptr<tr_peer_socket> socket,
    tr_bandwidth* parent,
    tr_sha1_digest_t const* info_hash,
    bool is_incoming,
    bool is_seed)
{
    ...
    auto const io = std::make_shared<tr_peerIo>(session, parent, info_hash, is_incoming, is_seed);
    ...
    bool utp)
{
    ...
    auto const get_socket = std::array<std::function<std::unique_ptr<tr_peer_socket>()>, TR_NUM_PREFERRED_TRANSPORT>{
        [&]() -> std::unique_ptr<tr_peer_socket>
        {
            if (utp)
            {
                return tr_peer_socket_utp::create(socket_address, session->utp_context);
            }
            return {};
        },
        [&]()
...
void tr_peerIo::set_socket(std::unique_ptr<tr_peer_socket> socket_in)
{
...
void tr_peerIo::write_cb()
{
    ...
    [[maybe_unused]] auto const n_wrote = try_write(SIZE_MAX);
...
size_t tr_peerIo::try_read(size_t max)
{
    ...
    // If there is no bandwidth left available, disable read.
    max = bandwidth().clamp(Dir, max);
    if (max == 0U)
    {
        set_enabled(Dir, false);
...
void tr_peerIo::read_cb()
{
    ...
    [[maybe_unused]] auto const n_read = try_read(n_left);
