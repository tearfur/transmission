...
{
    auto const pex = tr_pex::from_variant(std::data(l), std::size(l));
...
auto load_peers(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    if (auto const* l = map.find_if<tr_variant::Vector>(TR_KEY_peers2))
    {
        size_t const num_added = add_peers(tor, *l);
...
void save_labels(tr_variant::Map& map, tr_torrent const* tor)
{
    ...
    map.try_emplace(TR_KEY_labels, std::move(list));
}
...
tr_resume::fields_t load_labels(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    {
        if (auto osv = var.value_if<std::string_view>(); osv && !std::empty(*osv))
        {
            labels.emplace_back(*osv);
        }
    }
...
{
    map.try_emplace(TR_KEY_group, tr_variant::unmanaged_string(tor->bandwidth_group()));
}

tr_resume::fields_t load_group(tr_variant::Map const& map, tr_torrent* tor)
{
    if (auto const osv = map.value_if<std::string_view>(TR_KEY_group); osv && !std::empty(*osv))
    {
        tor->set_bandwidth_group(*osv);
...
void save_dnd(tr_variant::Map& map, tr_torrent const* tor)
{
    ...
    map.try_emplace(TR_KEY_dnd, std::move(list));
}
...
void save_file_priorities(tr_variant::Map& map, tr_torrent const* tor)
{
    ...
    map.try_emplace(TR_KEY_priority, std::move(list));
}
...
tr_resume::fields_t load_file_priorities(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    auto const n = tor->file_count();
    if (list == nullptr || n != std::size(*list))
    {
        return {};
    }
    ...
    for (tr_file_index_t i = 0; i < n; ++i)
    {
        if (auto const priority = (*list)[i].value_if<int64_t>())
        {
...
{
    map.try_emplace(TR_KEY_speed_limit_down, save_single_speed_limit(tor, TR_DOWN));
    map.try_emplace(TR_KEY_speed_limit_up, save_single_speed_limit(tor, TR_UP));
}
...
void save_ratio_limits(tr_variant::Map& map, tr_torrent const* tor)
{
    ...
    map.try_emplace(TR_KEY_ratio_limit, std::move(d));
}
...
void save_idle_limits(tr_variant::Map& map, tr_torrent const* tor)
{
    ...
    map.try_emplace(TR_KEY_idle_limit, std::move(d));
}

void load_single_speed_limit(tr_variant::Map const& map, tr_direction dir, tr_torrent* tor)
{
    if (auto const oi = map.value_if<int64_t>(TR_KEY_speed_Bps))
    {
        tor->set_speed_limit(dir, Speed{ *oi, Speed::Units::Byps });
    }
    else if (auto const oi2 = map.value_if<int64_t>(TR_KEY_speed))
    {
        tor->set_speed_limit(dir, Speed{ *oi2, Speed::Units::KByps });
    }

    if (auto const ob = map.value_if<bool>(TR_KEY_use_speed_limit))
    {
        tor->use_speed_limit(dir, *ob);
    }

    if (auto const ob = map.value_if<bool>(TR_KEY_use_global_speed_limit))
    {
        tr_torrentUseSessionLimits(tor, *ob);
    }
}
...
tr_resume::fields_t load_ratio_limits(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    if (auto const oi = d->value_if<int64_t>(TR_KEY_ratio_mode))
    {
        tor->set_seed_ratio_mode(static_cast<tr_ratiolimit>(*oi));
    }
...
tr_resume::fields_t load_idle_limits(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    if (auto const oi = d->value_if<int64_t>(TR_KEY_idle_mode))
    {
        tor->set_idle_limit_mode(static_cast<tr_idlelimit>(*oi));
    }
...
{
    map.try_emplace(TR_KEY_name, tr_variant::unmanaged_string(tor->name()));
}
...
tr_resume::fields_t load_filenames(tr_variant::Map const& map, tr_torrent* tor)
{
    ...
    {
        if (auto const osv = (*list)[i].value_if<std::string_view>(); osv && !std::empty(*osv))
        {
            tor->set_file_subpath(i, *osv);
        }
    }
...
void save_progress(tr_variant::Map& map, tr_torrent::ResumeHelper const& helper)
{
    ...
    map.try_emplace(TR_KEY_progress, std::move(prog));
}
...
tr_resume::fields_t load_progress(tr_variant::Map const& map, tr_torrent* tor, tr_torrent::ResumeHelper& helper)
{
    ...
    // try to load mtimes
    if (auto const* l = prog->find_if<tr_variant::Vector>(TR_KEY_mtimes))
    {
    ...
    if (auto const osv = prog->value_if<std::string_view>(TR_KEY_pieces))
    {
        raw_to_bitfield(checked, *osv);
    }

    // maybe it's a .resume file from [2.20 - 3.00] with the per-piece mtimes
    if (auto const* l = prog->find_if<tr_variant::Vector>(TR_KEY_time_checked))
    {
        for (tr_file_index_t fi = 0, n_l = std::size(*l); fi < n_files && fi < n_l; ++fi)
        {
            auto const& b = (*l)[fi];
            ...
            else if (auto const* ll = b.get_if<tr_variant::Vector>())
            {
                ...
                for (tr_piece_index_t i = 1, n_pieces = piece_end - piece_begin; i <= n_pieces && i < n_ll; ++i)
                {
    ...
    {
        if (auto const osv = b->second.value_if<std::string_view>())
        {
            raw_to_bitfield(blocks, *osv);
        }
        else
        {
    ...
    else if (auto const o_raw = prog->value_if<std::string_view>(TR_KEY_bitfield))
    {
        blocks.set_raw(reinterpret_cast<uint8_t const*>(std::data(*o_raw)), std::size(*o_raw));
    }
    else
    {
...
tr_resume::fields_t load_from_file(tr_torrent* tor, tr_torrent::ResumeHelper& helper, tr_resume::fields_t fields_to_load)
{
    ...
    if (auto oi = map.value_if<int64_t>(TR_KEY_corrupt); oi && (fields_to_load & tr_resume::Corrupt) != 0)
    {
        tor->bytes_corrupt_.set_prev(*oi);
        fields_loaded |= tr_resume::Corrupt;
    }

    if (auto osv = map.value_if<std::string_view>(TR_KEY_destination);
        osv && !std::empty(*osv) && (fields_to_load & (tr_resume::Progress | tr_resume::DownloadDir)) != 0)
    {
        helper.load_download_dir(*osv);
        fields_loaded |= tr_resume::DownloadDir;
    }

    if (auto osv = map.value_if<std::string_view>(TR_KEY_incomplete_dir);
        osv && !std::empty(*osv) && (fields_to_load & (tr_resume::Progress | tr_resume::IncompleteDir)) != 0)
    {
        helper.load_incomplete_dir(*osv);
        fields_loaded |= tr_resume::IncompleteDir;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_downloaded); oi && (fields_to_load & tr_resume::Downloaded) != 0)
    {
        tor->bytes_downloaded_.set_prev(*oi);
        fields_loaded |= tr_resume::Downloaded;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_uploaded); oi && (fields_to_load & tr_resume::Uploaded) != 0)
    {
        tor->bytes_uploaded_.set_prev(*oi);
        fields_loaded |= tr_resume::Uploaded;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_max_peers); oi && (fields_to_load & tr_resume::MaxPeers) != 0)
    {
        tor->set_peer_limit(static_cast<uint16_t>(*oi));
        fields_loaded |= tr_resume::MaxPeers;
    }

    if (auto ob = map.value_if<bool>(TR_KEY_paused); ob && (fields_to_load & tr_resume::Run) != 0)
    {
        helper.load_start_when_stable(!*ob);
        fields_loaded |= tr_resume::Run;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_added_date); oi && (fields_to_load & tr_resume::AddedDate) != 0)
    {
        helper.load_date_added(static_cast<time_t>(*oi));
        fields_loaded |= tr_resume::AddedDate;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_done_date); oi && (fields_to_load & tr_resume::DoneDate) != 0)
    {
        helper.load_date_done(static_cast<time_t>(*oi));
        fields_loaded |= tr_resume::DoneDate;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_activity_date); oi && (fields_to_load & tr_resume::ActivityDate) != 0)
    {
        tor->set_date_active(*oi);
        fields_loaded |= tr_resume::ActivityDate;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_seeding_time_seconds); oi && (fields_to_load & tr_resume::TimeSeeding) != 0)
    {
        helper.load_seconds_seeding_before_current_start(*oi);
        fields_loaded |= tr_resume::TimeSeeding;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_downloading_time_seconds);
        oi && (fields_to_load & tr_resume::TimeDownloading) != 0)
    {
        helper.load_seconds_downloading_before_current_start(*oi);
        fields_loaded |= tr_resume::TimeDownloading;
    }

    if (auto oi = map.value_if<int64_t>(TR_KEY_bandwidth_priority);
        oi && (fields_to_load & tr_resume::BandwidthPriority) != 0 && tr_isPriority(static_cast<tr_priority_t>(*oi)))
    {
        tr_torrentSetPriority(tor, static_cast<tr_priority_t>(*oi));
        fields_loaded |= tr_resume::BandwidthPriority;
    }

    if (auto ob = map.value_if<bool>(TR_KEY_sequentialDownload); ob && (fields_to_load & tr_resume::SequentialDownload) != 0)
    {
        tor->set_sequential_download(*ob);
