...
std::optional<std::vector<address_range_t>> parseFile(std::string_view filename)
{
    ...
    while (std::getline(in, line))
    {
        ...
        // ignore empty lines
        if (std::empty(line) || std::all_of(std::begin(line), std::end(line), [](char c) { return std::isspace(c); }))
        {
            continue;
        }
...
std::optional<size_t> Blocklists::update_primary_blocklist(std::string_view external_file, bool is_enabled)
{
