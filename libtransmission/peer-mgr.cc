...
public:
    ...
    void remove_peer(std::shared_ptr<tr_peerMsgs> peer)
    {
        auto const lock = unique_lock();
    ...
    tr_peer_id_t peer_id)
{
    ...
    auto const msgs = tr_peerMsgs::create(tor, std::move(peer_info), std::move(io), peer_id, &tr_swarm::peer_callback_bt, swarm);
...
void close_peer(std::shared_ptr<tr_peerMsgs> peer)
{
    TR_ASSERT(peer);
...
constexpr struct
{
    ...
    [[nodiscard]] bool operator()(std::shared_ptr<tr_peerMsgs> const& a, std::shared_ptr<tr_peerMsgs> const& b) const // less than
    {
        return compare(a, b) < 0;
    }
